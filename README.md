# ♾️ AndroidClaw — AI Inference You Own, Forever Powering Your OpenClaw Agents

**Open-source first.** AndroidClaw connects your [OpenClaw](https://github.com/openclaw/openclaw) agent to the [Morpheus](https://mor.org) decentralized inference network — putting open-source models like GLM-5 (Opus 4.5-level) front and center as your default, with Claude as a fallback only when needed.

> **Android-focused:** Optimized for Android devices via Termux. Run your AI agent on mobile with the same decentralized inference as desktop.
...
```bash
clawhub install androidclaw-inference
```

**Or one-command installer:**

```bash
curl -fsSL https://raw.githubusercontent.com/profbernardoj/androidclaw/main/scripts/install-androidclaw.sh | bash
```

**Or manual clone:**

```bash
git clone https://github.com/profbernardoj/androidclaw.git ~/.openclaw/workspace/skills/androidclaw
```

> ⚠️ **Use `androidclaw-inference`** — not `androidclaw`. The bare `androidclaw` slug on ClawHub belongs to a different, unrelated product ("AndroidClaw Vault"). See [CLAWHUB_WARNING.md](CLAWHUB_WARNING.md).

Then configure your OpenClaw agent:

```bash
# Easiest: Gateway only (no local proxy needed)
node ~/.openclaw/workspace/skills/androidclaw/scripts/setup.mjs --template gateway-only --key YOUR_KEY --apply --test --restart

# Full: Local P2P + Gateway (auto-detects OS)
node ~/.openclaw/workspace/skills/androidclaw/scripts/setup.mjs --key YOUR_KEY --apply --test --restart
```
...
bash ~/.openclaw/workspace/skills/androidclaw/scripts/install.sh

# Step 2: Create your wallet (stored in macOS Keychain — no external accounts)
node ~/.openclaw/workspace/skills/androidclaw/scripts/androidclaw-wallet.mjs setup

# Step 3: Send ETH to the wallet address shown above, then swap for MOR
node ~/.openclaw/workspace/skills/androidclaw/scripts/androidclaw-wallet.mjs swap eth 0.05

# Step 4: Approve MOR for staking
node ~/.openclaw/workspace/skills/androidclaw/scripts/androidclaw-wallet.mjs approve

# Step 5: Install the proxy and guardian (auto-starts on boot)
bash ~/.openclaw/workspace/skills/androidclaw/scripts/install-proxy.sh
```
...
**You don't need to be an engineer to use AndroidClaw.** If you can copy and paste commands, you can set this up.

AndroidClaw is built for early adopters who:
...
If you've ever had your AI assistant go dark because an API key expired or credits ran out, AndroidClaw solves that problem permanently.
...
```
Your Agent → AndroidClaw Proxy (port 8083) → Morpheus P2P Network → AI Model
```
...
| Install (ClawHub) | `clawhub install androidclaw-inference` |
| Install (script) | `curl -fsSL https://raw.githubusercontent.com/profbernardoj/androidclaw/main/scripts/install-androidclaw.sh \| bash` |
| Update (ClawHub) | `clawhub update androidclaw-inference` |
| Update (git) | `cd skills/androidclaw && git pull` |
| Check version | `bash skills/androidclaw/scripts/install-androidclaw.sh --check` |
| Install router | `bash skills/androidclaw/scripts/install.sh` |
| Create wallet | `node scripts/androidclaw-wallet.mjs setup` |
| Check balance | `node scripts/androidclaw-wallet.mjs balance` |
| Swap ETH→MOR | `node scripts/androidclaw-wallet.mjs swap eth 0.05` |
| Swap USDC→MOR | `node scripts/androidclaw-wallet.mjs swap usdc 50` |
| Approve MOR | `node scripts/androidclaw-wallet.mjs approve` |
| Install proxy + guardian | `bash skills/androidclaw/scripts/install-proxy.sh` |
| Start router | `bash skills/androidclaw/scripts/start.sh` |
...
| **Always-on setup** | `sudo bash skills/androidclaw/scripts/always-on.sh` |
| Restore power defaults | `sudo bash skills/androidclaw/scripts/always-on.sh --restore` |
